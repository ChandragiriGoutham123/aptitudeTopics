...
for i in range(number_of_inputs):
    direction_of_stream_towards_boat = input("enter direction of stream as  'against' : ").lower()
    ...
    if direction_of_stream_towards_boat == "against":
        ...
        if distance == "?":
            ...
            if total_time >= 0:
                ...
                print(" ")
                print("Time : "+str(total_time)+" hr"+" ---------- "+str(i+1))
                print(" ")
            else:
                print("Invalid details")
        elif speed_of_boat == "?":
            ...
            speed_of_stream = Fraction(speed_of_stream)
            distance = Fraction(distance)
            print("i.e speed of boat=(Distance/Time)+speed of stream")
            ...
            speed_of_boat = Fraction(speed_of_boat)
            distance = Fraction(distance)
            print("i.e speed of stream=speed_of_boat-(Distance/Time)")
            total_speed_of_stream = float(round(speed_of_boat-(distance/time), 2))
...
find_in_other_distances = input("do you want to find time in other distance in  downstream 'yes' or 'no' : ").lower()
if find_in_other_distances == "yes":
    if len(speeds_boats) > 0:
        another_distance = Fraction(input("enter another distance (km/hr): "))
        total_time_in_another_distance = round(float(another_distance / (speeds_boats[i-1]+speed_of_stream)), 2)
        print("Time (hr) : "+str(total_time_in_another_distance))
    elif len(speeds_stream) > 0:
        another_distance = Fraction(input("enter another distance (km/hr): "))
        total_time_in_another_distance = round(float(another_distance / (speed_of_boat+ speeds_stream[i-1])), 2)
        print("Time (hr) : " + str(total_time_in_another_distance))
